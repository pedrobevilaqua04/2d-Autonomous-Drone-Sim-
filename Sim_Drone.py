...
lidar_frontal = 0 # Declarações iniciais das variáveis
lidar_altura = 0
posicao_x = 0
posicao_y = 0
inclinacao = 0
...
def dar_mortal():
     global posicao_x, posicao_y, inclinacao, p_mot_EF, p_mot_DF, p_mot_ET, p_mot_DT

     p_mot_EF, p_mot_DF, p_mot_ET, p_mot_DT = 20, 20, 100, 100 # Potência para dar o mortal (Giro no eixo X)
     print(f"Dando mortal... Pitch-Front bem forte, Potências dos motores: EF={p_mot_EF}%, DF={p_mot_DF}%, ET={p_mot_ET}%, DT={p_mot_DT}%")
     print()
     inclinacao += 180 # Seria o sensor de inclinação do Drone
     print(f"Inclinação atual no meio do mortal: {inclinacao}°")
     print()
     inclinacao += 180
     print(f"Inclinação atual no final do mortal: {inclinacao}°")
     print()
     p_mot_EF, p_mot_DF, p_mot_ET, p_mot_DT = 50, 50, 50, 50 # Potência de estabilização
     print(f"Voltando a estabilizar após o mortal... Potências dos motores: EF={p_mot_EF}%, DF={p_mot_DF}%, ET={p_mot_ET}%, DT={p_mot_DT}%")
     print(f"Inclinação final normalizada: {inclinacao % 360}°")
     print()
     input("Pressione Enter para continuar...")
     print("----------------------------------")
...
inicializacao()
takeOff()
while posicao_x != 20: #Realizar leitura de sensores e movimentos enquanto o drone não chegar na parede final
    ver_mov_front()
dar_mortal()
land()
